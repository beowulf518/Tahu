...
class ItemDetails extends Component {
    ...
    async componentDidMount(){
        ...
        const path = '/item/' + this.props.id;
        const myInit = {
            headers: {},
        };
        const res = await API.get(apiName, path, myInit);
        console.log("Get Item in details By Id : ", res);

        initData.title = res.title;
        initData.content = res.description;
        initData.isVideo = res.previewContentType.includes('video') ? true: false;
        initData.sources = [{
                                src: res.previewUrl,
                                type: res.previewContentType
                            }];
        initData.url = res.previewUrl;
                        ...
                        <div className="col-12 col-lg-5">
                            <div className="item-info">
                                <div className="item-thumb text-center">
                                    ...
                                    {this.state.initData.isVideo && <VideoJS options={this.state.initData} />}
                                    {!this.state.initData.isVideo && <img className="card-img-top" src={this.state.initData.url} alt="" /> }
                                </div>
